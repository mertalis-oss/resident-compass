...
export default function ServiceCheckout({ service, variant = "full", layout = "standalone" }: Props) {
  ...
  if (isPlaceholder) {
    console.warn(
      "STRIPE PLACEHOLDER DETECTED:",
      service.stripe_price_id,
      "— Real price_id needed from Stripe Dashboard.",
    );
  }
  ...
  const {
    ...
    isPrivate,
    resolvedCurrency,
  } = renderPrice(service.price, service.currency || "USD", scope);
      ...
      <Dialog open={modalOpen} onOpenChange={handleModalChange}>
        ...
        >
          ...
          <div className="max-h-[80vh] overflow-y-auto p-6 md:p-8 pb-32">
            ...
            <p className="text-xs text-muted-foreground ml-7 mb-2">{t("checkout.legalReinforce")}</p>
            {!isAgreed && <p className="text-xs text-destructive/70 ml-7 mb-6">{t("checkout.mustAccept")}</p>}
            ...
            <div className="text-center mt-6 space-y-1.5">
              ...
              <button
                ...
                className="text-xs text-accent hover:text-accent/80 underline underline-offset-4 transition-colors inline-flex items-center gap-1.5"
              >
            ...
            <div className="mt-8 space-y-4">
              <p className="caption-editorial text-muted-foreground mb-4">{t("checkout.whatsNextLabel")}</p>
            ...
            {/* Real human */}
            <p className="text-sm text-muted-foreground text-center mt-8 leading-relaxed">{t("checkout.realHuman")}</p>
